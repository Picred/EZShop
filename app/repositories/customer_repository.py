...
class CustomerRepository:
        ...
        async with await self._get_session() as session:
            ...
            if card is None:
                ...
                result_conflict = await session.execute(
                    select(CustomerDAO).filter(CustomerDAO.cardId == cardId)
                )
    ...
    async def list_customer(self) -> list[CustomerDAO]:
        ...
        async with await self._get_session() as session:
            result = await session.execute(
                select(CustomerDAO).options(selectinload(CustomerDAO.card))
            )
    ...
    async def update_customer_only_name(
        self, customer_id: int, updated_name: str
    ) -> CustomerDAO | None:
        """
    ...
    async def update_customer(
        ...
        async with await self._get_session() as session:
            ...
            if updated_cardId == "" and updated_points == -1:
                ...
                result_conflict = await session.execute(
                    select(CustomerDAO).filter(CustomerDAO.cardId == updated_cardId)
                )
                result_conflict = result_conflict.scalars().first()
                if (
                    result_conflict is not None and result_conflict.id != db_customer.id
                ):  # if there is a conflict and the customer id are same
                    raise ConflictError(
                        "Card with id "
                        + updated_cardId
                        + " is already attached to a customer"
                    )
    ...
    async def create_card(self) -> CardDAO:
        ...
        async with await self._get_session() as session:
            id_db = await session.execute(
                select(CardDAO).order_by(desc(CardDAO.cardId))
            )
    ...
    async def attach_card(self, customer_id: str, card_id: str) -> CustomerDAO:
        ...
        async with await self._get_session() as session:
            ...
            customer = await session.execute(
                select(CustomerDAO).filter(customer_id == CustomerDAO.id)
            )
            customer = customer.scalars().first()
            result_conflict = await session.execute(
                select(CustomerDAO).filter(CustomerDAO.cardId == card_id)
            )
            result_conflict = result_conflict.scalars().first()
            ...
            if result_conflict is not None:
                raise ConflictError(
                    "Card with id " + card_id + " is already attached to a customer"
                )
